var __defProp=Object.defineProperty;var __name=(target,value)=>__defProp(target,"name",{value,configurable:!0});import{r as requireElement}from"./requiredQuerySelector.js";const scriptObserver=new MutationObserver(nodes=>{nodes.forEach(node=>{node instanceof HTMLScriptElement&&node.src.includes("game.js")&&(node.addEventListener("beforescriptexecute",e=>e.preventDefault(),{once:!0}),node.src="",node.remove())})});scriptObserver.observe(document,{childList:!0,subtree:!0});const content=new Promise(async resolve=>{let html=await(await fetch(window.location.href)).text();html=html.replaceAll("game.js","game.jsx");const newDoc=document.createElement("html");newDoc.innerHTML=html;const body=requireElement("body",newDoc);body.dataset.typo_loader="true",resolve(body)}),loaded=new Promise(resolve=>{document.readyState==="complete"&&resolve(),document.addEventListener("DOMContentLoaded",()=>resolve())});loaded.then(async()=>{const body=await content;document.body=body,scriptObserver.disconnect(),signature()});const signature=__name(()=>{console.clear(),console.log(`%c
        _             _   _       _       _   _                           
